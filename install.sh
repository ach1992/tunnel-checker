...
packages=(ca-certificates curl iputils-ping iproute2 socat coreutils procps)
missing=()
...
for source in api raw cdn; do
  ...
  case $source in
    api)
      ...
      curl -fsSL --connect-timeout 8 --max-time 30 -H 'Accept: application/vnd.github.raw+json' -H 'X-GitHub-Api-Version: 2022-11-28' -H 'User-Agent: tunnel-checker' "$API_URL" -o "$tmp" || continue
      ;;
    raw)
  ...
  if [[ -s $tmp ]] && bash -n "$tmp"; then success=1; break; fi
done
((success==1)) || fail "Could not download a valid Tunnel Checker script."
